(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('schedulingProjectApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-schedulingProjectApp-error');
                var entityKey = httpResponse.headers('X-schedulingProjectApp-params');
                if (errorHeader) {
